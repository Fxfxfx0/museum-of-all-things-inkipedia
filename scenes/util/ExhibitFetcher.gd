...
var search_endpoint = "https://splatoonwiki.org/w/api.php?action=query&format=json&list=search&srprop=title&origin=*&srsearch="
var random_endpoint = "https://splatoonwiki.org/w/api.php?action=query&format=json&generator=random&grnnamespace=0&prop=info&origin=*"

var wikitext_endpoint = "https://splatoonwiki.org/w/api.php?action=query&prop=revisions|extracts|pageprops&ppprop=wikibase_item&explaintext=true&rvprop=content&format=json&redirects=1&origin=*&titles="
var images_endpoint = "https://splatoonwiki.org/w/api.php?action=query&prop=imageinfo|revisions&iiprop=extmetadata|url&iiurlwidth=640&iiextmetadatafilter=LicenseShortName|Artist&rvprop=content&format=json&redirects=1&origin=*&titles="
...
func _dispatch_request(url, ctx, caller_ctx):
  ...
  var result
  if Util.is_using_threads():
    result = RequestSync.request(url)
  else:
    result = await RequestSync.request_async(url)
